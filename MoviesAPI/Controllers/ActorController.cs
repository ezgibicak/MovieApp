...
namespace MoviesAPI.Controllers
{
    ...
    public class ActorController : ControllerBase
    {
        ...
        public ActorController(ILogger<ActorController> logger, ApplicationDbContext context, IMapper mapper, IFileStorageService fileStorageService)
        {
            this.logger = logger;
        ...
        public async Task<ActionResult<List<ActorsMovieDTO>>> SearchByName(string query)
        {
            ...
            var actors = await context.Actors.Where(x => x.Name.Contains(query)).OrderBy(x => x.Name).Select(x => new ActorsMovieDTO { Id = x.Id, Name = x.Name, Picture = x.Picture }).Take(5).ToListAsync();
            return actors;
        }
    }
}
